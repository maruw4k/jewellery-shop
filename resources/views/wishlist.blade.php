...
@section('content')
    ...
    <div class="container">
        <p><a href="{{ url('shop') }}">Główna</a> / Lista życzeń</p>
        ...
        @if (sizeof(Cart::instance('wishlist')->content()) > 0)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ url('switchToCart', [$item->rowId]) }}" method="POST" class="side-by-side">
                                {!! csrf_field() !!}
                                <input type="submit" class="btn btn-success btn-sm" value="Do koszyka">
                            </form>
                        </td>
                    </tr>
                    @endforeach
            ...
            <a href="/shop" class="btn btn-primary btn-lg">Kontynuuj zakupy</a> &nbsp;

            <div style="float:right">
                <form action="{{ url('/emptyWishlist') }}" method="POST">
                    {!! csrf_field() !!}
                    <input type="hidden" name="_method" value="DELETE">
                    <input type="submit" class="btn btn-danger btn-lg" value="Pusta lista życzeń">
                </form>
            </div>

        @else

            <h3>Nie masz nic na liście życzeń</h3>
            <a href="/shop" class="btn btn-primary btn-lg">Kontynuuj zakupy</a>
